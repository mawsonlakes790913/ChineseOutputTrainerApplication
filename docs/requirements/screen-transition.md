...
G26["G26 パスワード再設定メール送信画面"]
G27["G27 パスワード再設定メール送信完了画面"]
G28["G28 パスワード再設定画面"]
...
アカウントを凍結した場合、対象ユーザーの情報および関連データは保持したままログイン不可とする。

また、ログイン中の管理者自身に対する凍結および削除は行えないものとする。

これらの操作はG19 アドミン専用ユーザー管理画面内で行うため、凍結・凍結解除・削除のための個別画面は設けない。
...
L --> FP["G26 パスワード再設定メール送信"]
FP --> FS["G27 パスワード再設定メール送信完了"]
...
また、現時点ではこれらの設定をアプリケーション上から変更する必要性がないため、G20 アドミン専用AI設定画面は実装しない方針とした。

そのため、G20およびG20への画面遷移を画面遷移図から削除した。
...
廃止によって欠番となっていたG20を、メールアドレス変更画面として再利用する。

旧G20「アドミン専用AI設定画面」と、現在のG20「メールアドレス変更画面」に機能上の関連はない。

---

## 13. パスワードリセット

パスワードを忘れたユーザーは、ログイン画面からパスワード再設定を開始できる。
...
G26では登録済みのメールアドレスを入力し、パスワード再設定用メールの送信を要求する。

メールにはパスワード再設定用URLを記載する。

ユーザーがメール内のURLへアクセスすると、G28 パスワード再設定画面へ遷移する。
...
パスワード再設定画面は、有効なパスワードリセットトークンを使用してアクセスする。

トークンの有効期限が切れている場合、または有効なトークンを確認できない場合は、パスワードを再設定できない。

パスワード再設定が完了した場合は、使用したパスワードリセットトークンを無効とする。