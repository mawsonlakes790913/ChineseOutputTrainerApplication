...
これにより、同一の生成元QUESTIONから問題を繰り返し生成した場合でも、直近の生成結果と同一または過度に類似した問題が短期間に繰り返し生成されることを抑制する。

ただし、AI_GENERATION_HISTORYは過去の生成結果に含まれる個々の語句を使用禁止とするためのものではない。

同じ語句が一部含まれていても、文章全体として十分な変化があり、自然な中国語として成立する場合は再利用を許可する。
